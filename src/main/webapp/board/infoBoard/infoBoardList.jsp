...
<div class="bg-light py-3" style="color: black;">
    <div class="container">
        <div class="row">
            <div class="col-md-12 mb-0" style="font-size: 15px; font-weight: bold;">커뮤니티 게시판 > 정보게시판</div>
        </div>
    </div>
</div>
	        ...
	        <td style="width: 30%;">
	            <div style="display: table; height: 100%; width: 100%;">
	                <div style="display: table-cell; vertical-align: middle;">
	                    ...
	                    <a href="${path}/front?key=board&methodName=selectByInfoBoardSeq&boardSeq=${boardDTO.boardSeq}" style="color: black;">
...
<c:if test="${not empty loginUser}">
		...
		<div align=right>
			...
			<a href="${path}/board/infoBoard/infoBoardWrite.jsp" class="btn">글쓰기</a>
			</span>
		</div>
		<hr>
</c:if>
...
<nav class="pagination-container">
	...
	\${pageNo} = ${pageNo} , 	\${temp}=${temp}  ,   \${startPage}=${startPage}  , <br>
		  ...
		  <c:if test="${(startPage-p.blockcount) > 0}"> <!-- (-2) > 0  -->
		      <a class="pagination-newer" href="${path}/front?key=board&methodName=selectAllInfoBoard&pageNo=${startPage-1}">◀</a>
		  </c:if>
				...
				<span class="pagination-inner"> 
				  <c:forEach var='i' begin='${startPage}' end='${(startPage-1)+p.blockcount}'> 
					  ...
					  <c:if test="${not doneLoop}" >
					         <a class="${i==pageNo?'pagination-active':page}" href="${path}/front?key=board&methodName=selectAllInfoBoard&pageNo=${i}">${i}</a> 
					  </c:if>
				</c:forEach>
				</span> 
				
				 <c:if test="${(startPage+p.blockcount)<=p.pageCnt}">
				     <a class="pagination-older" href="${path}/front?key=board&methodName=selectAllInfoBoard&pageNo=${startPage+p.blockcount}">NEXT</a>
				 </c:if>
